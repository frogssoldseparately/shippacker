...
func Pack(musicSrcPath string, outPath string) error {
	...
	filesWritten, banksWritten, err := WriteModEntries(musicSrcPath, outPath, localW, centralW, assetMap, 0, globals.StartingBankIndex)
	if err != nil {
		return err
	}
...
func WriteModEntries(srcPath string, outPath string, lw *swriter.SimpleWriter, cw *swriter.SimpleWriter, assetMap *maps.Assets, startFileCount uint16, startBankId uint64) (uint16, uint64, error) {
	...
	for _, file := range files {
		if globals.RecurseSubdirectories && file.IsDir() {
			fmt.Printf("Stepping into %s\n", file.Name())
			newCount, newBanks, err := WriteModEntries(filepath.Join(srcPath, file.Name()), outPath, lw, cw, assetMap, includedFileCount, bankId)
			fmt.Printf("Stepping out of %s\n", file.Name())
			if err != nil {
				return 0, 0, err
			}
